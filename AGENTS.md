...
- 新建渠道自动按规则处理（可选开启）
- 通知时间线（摘要级，默认弱采集；记包名/应用名/渠道名/id/时间范围/条数/importance，**不记正文**；1 分钟合并窗，7 天 / 200 条；记录页按日期与 App 分组 + 今日摘要 + 静音联动）。**最近任务划掉后的恢复分层**：默认无常驻——打开 App / 记录页补绑 + `BOOT_COMPLETED`/`MY_PACKAGE_REPLACED` 一次性 rebind + 设置页 ROM 生存引导；NLS/健康检查 Job/可选 FGS 跑在 `:listener` 进程；可选「低频监听健康检查」（JobScheduler ~20min，默认关）；可选「后台持续采集」FGS（默认关）；跨进程开关与连接状态走 `ListenerFlagStore` 文件
