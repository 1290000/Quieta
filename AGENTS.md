...
- 规则本地存储，可导入/导出 JSON（根字段 `schemaVersion` + `kind: "rules"`；配置页勾选子集或全部导出；导入预览后可选合并/替换）
...
8. ~~**记录页筛选**~~ 以外的备份边界：**规则导出**配置页支持多选/全部分享 JSON，文件导入可**合并**或**替换**；**渠道通知设置快照**（按 App 导出/跨机导入）尚未实现。
