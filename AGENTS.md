...
- 规则本地存储，可导入/导出 JSON（根字段 `schemaVersion` + `kind: "rules"`；配置页勾选子集或全部导出；导入预览后可选合并/替换）；**渠道设置快照**可导出 JSON（`kind: "quieta-channel-snapshot"`，包名+渠道 id 关联，importance 主字段；导入未做）
...
8. ~~**记录页筛选**~~ 以外的备份边界：**规则导出**配置页支持多选/全部分享 JSON，文件导入可**合并**或**替换**；**渠道设置快照**已支持导出（主页多选所选渠道 / 设置·备份导出全部盘点，JSON `kind: "quieta-channel-snapshot"`，字段以 importance 为主）；**跨机导入应用**（dry-run + setImportance + 回读）尚未实现。
